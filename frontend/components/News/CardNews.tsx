...
export default function CardNews({berita}:{berita:DetailBerita}) {
  return (
    <Link href={`/berita/${berita.id}`}>
      <a className='group cursor-pointer bg-gradient-cust-orange2 hover:bg-none rounded-[10px] p-[.1rem] w-full min-[550px]:w-[80%] sm:max-w-none sm:w-[45%] lg:w-fit'>
        <article className='w-fit lg:w-[20rem] rounded-[8px] p-3 pb-8 flex flex-col flex-wrap lg:justify-center bg-white hover:bg-pastel '>
          <Image className='rounded-xl bg-cover' src={`${API_URL}${berita.cover.url}`} width={300} height={180} alt='News'/>
